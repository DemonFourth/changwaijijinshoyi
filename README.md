...
#### FIFO成本法
- **持仓收益** = 当前市值 - 持仓成本
- **已实现收益** = 各笔卖出到手金额 - 对应FIFO成本
- **总收益** = 持仓收益 + 已实现收益
- **收益率** = 总收益 / 总投入(净值×份额+手续费) × 100%

#### 简单差额法
- **收益** = (已卖出到手总额 + 持仓市值) - 买入成本总额(净值×份额)
- **收益率** = 收益 / 买入成本总额 × 100%

> 简单差额法需将持仓市值计入，否则持仓中的基金会显示为负收益。已清仓时持仓市值为0，与FIFO法结果一致。

### 已清仓基金处理

当持有份额 ≤ 0.0001（精度阈值EPSILON）时视为已清仓：
- 持有份额、持仓成本、当前市值、浮动盈亏均归零
- 每份成本显示 ¥0.00
- 持仓收益率显示 "-"
- 持仓周期状态标记为"已结束"
